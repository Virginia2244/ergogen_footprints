    ...
    */
    ...
    const get_thru_hole = () => {
      ...
      for (let i = 0; i < spacing.total_pin_num/2; i++) {
        thru_hole += `(pad ${i}                             thru_hole oval (at ${spacing.top_left_pin.x}  ${spacing.top_left_pin.y + (i)*spacing.pin_dist}  ${p.rot})       (size 1.7 1.7) (drill 1) (layers *.Cu *.Mask) ${p.reversable ? (i < 4? p.local_net(i).str : pin_nets[i][0]) : pin_nets[i][0]})\n`
        thru_hole += `(pad ${spacing.total_pin_num - 1 - i} thru_hole oval (at ${spacing.top_right_pin.x} ${spacing.top_right_pin.y + (i)*spacing.pin_dist} ${180 + p.rot}) (size 1.7 1.7) (drill 1) (layers *.Cu *.Mask) ${p.reversable ? (i < 4? p.local_net(spacing.total_pin_num - 1 - i).str : pin_nets[i][1]) : pin_nets[i][1]})\n`
      }
      return thru_hole
    }
